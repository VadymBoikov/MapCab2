...
@app.route('/realtime')
def realtime():
    conn = happybase.Connection('10.128.0.7')
    table = conn.table('map_online')
    row = table.row('city')
    cabs = []
    for key, val in row.iteritems():
        dval = json.loads(val)
        cabs.append({'name':key.split(':')[1], 'lat': dval['lat'], 'lng': dval['lng']})
    return jsonify(cabs=cabs)

@app.route('/doworder')
def doworder():
    hbase = happybase.Connection('10.128.0.7')
    ...
    for i, day in enumerate(keys):
        ...
        cellval = json.loads(row['data:Totals'])
...
def hodorder(day):
    ...
    for col in cols:
        ...
        results[hour]['pickups'] = round(float(cellval['picks'])/totalCabs, 2)
        results[hour]['dropoffs'] = round(float(cellval['drops'])/totalCabs, 2)
        results[hour]['occ'] = round(float(cellval['occ'])*100, 2)
        results[hour]['dist'] = round(float(cellval['distance'])/totalCabs, 2)
